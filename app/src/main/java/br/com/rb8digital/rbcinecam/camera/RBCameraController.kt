...
import br.com.rb8digital.rbcinecam.monitoring.ScopeAnalyzer
...
class RBCameraController(private val context: Context) {
    ...
    fun bind(
        ...
        if (onScopeFrame != null) scopeCallback = onScopeFrame
        ...
        providerFuture.addListener({
            ...
            val analysis = scopeCallback?.let { callback ->
            ...
            camera = if (analysis != null) {
                provider.bindToLifecycle(owner, selector, preview, videoCapture, imageCapture, analysis)
            } else {
                provider.bindToLifecycle(owner, selector, preview, videoCapture, imageCapture)
            }
    ...
    fun focusSingle(previewView: PreviewView): String {
        ...
        activeCamera.cameraControl.startFocusAndMetering(FocusMeteringAction.Builder(point, FocusMeteringAction.FLAG_AF).build())
        return "AF-S"
    }
    ...
    fun focusLock(previewView: PreviewView): String {
        ...
        val point = previewView.meteringPointFactory.createPoint(previewView.width / 2f, previewView.height / 2f)
        activeCamera.cameraControl.startFocusAndMetering(
            FocusMeteringAction.Builder(point, FocusMeteringAction.FLAG_AF).disableAutoCancel().build()
        )
        return "LOCK"
    }
    ...
    fun manualFocusSupported(): Boolean {
        ...
        return FocusControlPolicy.manualFocusSupported(
    ...
    fun setManualFocus(fraction: Float): String {
        ...
        return when (percent) { 0 -> "∞"; 100 -> "MACRO"; else -> "MF $percent%" }
    }
    ...
    fun cycleShutter(): String { shutterIndex = (shutterIndex + 1) % shutterAngles.size; applyManualControls(); return shutterLabels[shutterIndex] }
    fun cycleWhiteBalance(): String { wbIndex = (wbIndex + 1) % wbModes.size; applyManualControls(); return wbLabels[wbIndex] }
    fun cycleFocus(): String { manualFocusFraction = null; focusIndex = (focusIndex + 1) % focusFractions.size; applyManualControls(); return focusLabels[focusIndex] }
    ...
    fun cycleEv(): String {
        ...
        activeCamera.cameraControl.setExposureCompensationIndex(desiredIndex.coerceIn(range.lower, range.upper))
        return formatEv()
    }
    ...
    private fun resetManualState() {
        isoIndex = 0; shutterIndex = 0; wbIndex = 0; focusIndex = 0; evStops = 0; manualFocusFraction = null
    }
    ...
    private fun applyManualControls() {
        ...
        if (iso == null && shutterNs == null) {
            ...
            val safeShutter = (shutterNs ?: 16_666_667L).let { value -> exposureRange?.let { value.coerceIn(it.lower, it.upper) } ?: value }
        ...
        if (requestedManual != null) {
            val minFocus = cameraInfo.getCameraCharacteristic(CameraCharacteristics.LENS_INFO_MINIMUM_FOCUS_DISTANCE) ?: 0f
            if (FocusControlPolicy.manualFocusSupported(minFocus)) {
                builder.setCaptureRequestOption(CaptureRequest.CONTROL_AF_MODE, CaptureRequest.CONTROL_AF_MODE_OFF)
                builder.setCaptureRequestOption(CaptureRequest.LENS_FOCUS_DISTANCE, FocusControlPolicy.distanceForFraction(minFocus, requestedManual))
            }
        } else {
            ...
            if (focusFraction == null) {
                ...
                val minFocus = cameraInfo.getCameraCharacteristic(CameraCharacteristics.LENS_INFO_MINIMUM_FOCUS_DISTANCE) ?: 0f
                builder.setCaptureRequestOption(CaptureRequest.CONTROL_AF_MODE, CaptureRequest.CONTROL_AF_MODE_OFF)
                builder.setCaptureRequestOption(CaptureRequest.LENS_FOCUS_DISTANCE, FocusControlPolicy.distanceForFraction(minFocus, focusFraction))
            }
        }
